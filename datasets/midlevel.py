...
from datasets.dataset_utils import slice_func, normalize_spec, get_dataset_stats, slice_func_simple
...
class MidlevelDataset(DatasetBase):
    def __init__(self, **kwargs):
        ...
        self.mixup_fraction = kwargs.get('mixup_fraction', 0.0)
    ...
    def __getitem__(self, ind):
        ...
        do_mixup = True if np.random.rand(1) < self.mixup_fraction else False

        if do_mixup:
            song_id_2 = np.random.choice(self.song_id_list)
            audio_path_2 = self._get_audio_path_from_id(song_id_2)
            x_2 = self._get_spectrogram(audio_path_2, self.dataset_cache_dir)
            labels_2 = self._get_labels(song_id_2)
            x_sliced, labels = temporal_mixup(x, labels, x_2, labels_2, slice_length)
        else:
        ...
        return audio_path, torch.from_numpy(x_aug), labels
